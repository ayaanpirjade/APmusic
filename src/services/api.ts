...
export const api = {
  async getHomeModules(language = 'hindi,english,punjabi'): Promise<{
    ...
    const res = await fetch(`/api/modules?language=${encodeURIComponent(language)}`);
  ...
  async searchSongs(query: string, page = 1, limit = 20): Promise<Song[]> {
    const res = await fetch(`/api/search/songs?query=${encodeURIComponent(query)}&page=${page}&limit=${limit}`);
    const data = await res.json();
  ...
  async getSongSuggestions(id: string, limit = 15): Promise<Song[]> {
    const res = await fetch(`/api/songs/${encodeURIComponent(id)}/suggestions?limit=${limit}`);
    const data = await res.json();
  ...
  async getLyrics(
    ...
    const res = await fetch(`/api/lyrics/${encodeURIComponent(lyricsIdOrSongId || 'unknown')}${qs}`);
    const data = await res.json();
  ...
  async getSpotifyPresets(): Promise<SpotifyPreset[]> {
    const res = await fetch('/api/spotify/presets');
    const data = await res.json();
    if (!data.success) return [];
    return data.data;
  },
